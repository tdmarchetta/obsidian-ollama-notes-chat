...
**Shipped — 0.7.14** (published 2026-06-26, latest release; `main` at the #25 merge, 238 tests): chat-subheader UX. The click-to-cycle context pill became two native-`Menu` dropdowns — a **context picker** (all six modes, current checkmarked; selection stays session-only) and a **model picker** that lists installed models from `/api/tags` and switches `settings.model` live via the existing `saveSettings → notifyViews → refreshSubheader` path (no Settings trip). `ChatView.ts` only (`buildSubheaderSegment` + `openContextMenu`/`openModelMenu`/`selectModel`/`showMenuUnder`; `cycleContextMode` → `setContextMode`) + per-segment CSS. **Release lessons:** (1) `obsidianmd/ui/sentence-case` flags brand capitalization in UI strings — a `Notice` saying "Ollama" mid-sentence failed CI; use "the server" (codebase convention). (2) Local ESLint can't run on this working copy (the SynologyDrive mount's `node_modules` can't resolve `eslint-plugin-obsidianmd` → `@microsoft/eslint-plugin-sdl`) — typecheck/test/build run via `node`, but lint is **CI-only** here. (3) Published on green CI without the manual smoke test, at the maintainer's call — flagged as patch-if-needed.

Prior — **0.7.13** (published 2026-06-26, `#23`): note-name chat-history titles + one-time backfill. Auto-titles came from the first user message, so bare-`/summarize` chats all read "/summarize"; `deriveAutoTitle()` now falls back to the source note's basename (`src/util/noteBasename.ts` + tests), and a load-time backfill re-titles existing auto-named chats (manual `titleManuallySet` untouched; null re-derivation skipped so no title is wiped).

Prior — **0.7.12** (published 2026-06-17; `main` at the #15 merge, 229 tests): maintenance pass. Removed the four deprecated `SliderComponent.setDynamicTooltip()` calls in `SettingsTab.ts` (value shows inline as of obsidian 1.13.x; the 1.13.1 types mark it `@deprecated`) — clears the community-dashboard recommendation and **unblocked the obsidian 1.13.0→1.13.1 dev-dep bump** (#8, merged right after), whose `CI / check` had failed on those exact four lines via `@typescript-eslint/no-deprecated`. Dead-code sweep found the `.ts` source clean but ~50 stray compiled `.js` next to the `.ts` (untracked, not gitignored) from a bare `tsc` run — fixed at the root with `noEmit: true` in `tsconfig` (esbuild does the bundling; `tsc` is typecheck-only) plus a `.gitignore` backstop (`src|test/**/*.js`, `vitest.config.js`). Also landed green dependabot bumps #4 (actions/checkout 6.0.3), #5 (typescript-eslint 8.61.1), #6 (@typescript-eslint/parser 8.61.1). **Release lesson:** a dev-dep type bump can surface a *new* `@typescript-eslint/no-deprecated` error (here obsidian 1.13.1 deprecating `setDynamicTooltip`) — the fix is to remove the deprecated call, which also unblocks the bump.
